...
class TestValueResolverConsolidation(FrappeTestCase):
	...
	def test_value_source_resolver(self):
		res_field = ValueResolver.compile(
			{"kind": "value_source", "operation": "field", "path": "doc.status"}
		)
		self.assertEqual(res_field.resolve(self.context), "Active")

		res_var = ValueResolver.compile(
			{"kind": "value_source", "operation": "variable", "path": "vars.discount"}
		)
		self.assertEqual(res_var.resolve(self.context), 50.0)

		res_static = ValueResolver.compile(
			{"kind": "value_source", "operation": "static", "value": "FixedValue"}
		)
